...
/**
 * Affiliations panel. Logos are transparent cream silhouettes that float
 * directly on the dark forest backdrop — no tiles, no title, no chrome.
 * Row 1: UIdaho · IFC · DFA.  Row 2: PSAE · CAFS.
 */
export function AffiliationsPanel() {
  const row1 = [
    ...
    { src: logoDfa.url, label: "Digital Forest Affiliates", scale: 1.35 },
  ];
  const row2 = [
  ...
  const Logo = ({ src, label, scale }: { src: string; label: string; scale: number }) => (
    <div className="relative flex items-center justify-center w-full h-[88px] sm:h-[100px]">
      <img
        src={src}
        alt={label}
        loading="lazy"
        style={{
          ...
          objectFit: "contain",
          opacity: 0.88,
          filter: "drop-shadow(0 0 18px rgba(95,217,154,0.12))",
  ...
  return (
    <div className="relative w-full max-w-[480px] mx-auto">
      {/* soft halo behind logos to lift them off the bg */}
      <span
        aria-hidden
